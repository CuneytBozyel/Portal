...
function geocode(position) {
    geocoder.geocode({
        ...
        if (responses && responses.length > 0) {
            html += 'Postal Address:<br/>' + responses[0].formatted_address;
            html += '<br /><small>' + 'Latitude: ' + marker.getPosition().lat() + '<br />Longitude: ' + marker.getPosition().lng();
            ...
            html += '<span style="float:right"><a target="_blank" href="https://twitter.com/intent/tweet?related=labnol&via=labnol&url=' + encodeURIComponent(window.location.href) + '&text=I%20am%20at%20' + encodeURIComponent(responses[0].formatted_address) + '">Tweet #location</a></span>';
        } else {
